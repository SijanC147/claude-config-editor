...
def get_index_html():
    ...
    try:
        # Python 3.9+
        package_files = files('claude_config_editor')
        return (package_files / 'index.html').read_text(encoding='utf-8')
    except (TypeError, AttributeError):
        # Python 3.7-3.8 fallback
        try:
            from importlib import resources
            if hasattr(resources, 'read_text'):
                return resources.read_text('claude_config_editor', 'index.html')
            else:
                # Fallback to pkg_resources or file-based approach
                raise ImportError("Need alternative method")
        except Exception:
            # Ultimate fallback: try to find the file relative to this module
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if config_type in configs:
            ...
            print(f"Available: {', '.join(configs.keys()) if configs else 'none found'}")
            sys.exit(1)
    else:
